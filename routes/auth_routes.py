...
def change_password():
    ...
    try:
        ...
        if not user_email:
            return _error_response('사용자 이메일을 확인할 수 없습니다.'), 400
        get_supabase().auth.sign_in_with_password({'email': user_email, 'password': current_password})
    except Exception:
        return _error_response('현재 비밀번호가 일치하지 않습니다.')
