...
def suggest_fixes(content: str) -> dict:
    """헤딩 계층 구조 이슈에 대한 구체적 수정 액션을 제안합니다.

    각 이슈에 대해 원본 → 수정 형태의 변경 제안을 생성합니다.

    Args:
        content: 마크다운 콘텐츠

    Returns:
        fixes (수정 제안 리스트), fix_count, auto_fixable_count를 포함하는 dict
    """
    if not content or not content.strip():
        return {'fixes': [], 'fix_count': 0, 'auto_fixable_count': 0}
    ...
    for issue in issues:
        if issue['type'] == 'skip':
            from_lv = issue['from_level']
            to_lv = issue['to_level']
            heading_text = issue['heading']
            target_level = from_lv + 1
            fixes.append({
                'type': 'level_change',
                'heading': heading_text,
                'original': '#' * to_lv + ' ' + heading_text,
                'suggested': '#' * target_level + ' ' + heading_text,
                'reason': f'H{from_lv}→H{to_lv} 건너뛰기 해소 (H{target_level}로 변경)',
                'auto_fixable': True,
            })
            ...
            fixes.append({
                ...
                'original': '',
                'suggested': '',
                'reason': f'"{issue["heading"]}" 섹션에 본문을 추가하거나 헤딩을 제거하세요',
                'auto_fixable': False,
            })

        elif issue['type'] == 'deep_nesting':
            heading_text = issue['heading']
            deep_h = next((h for h in headings if h['text'][:50] == heading_text), None)
            if deep_h:
                fixes.append({
                    'type': 'level_change',
                    'heading': heading_text,
                    'original': '#' * deep_h['level'] + ' ' + heading_text,
                    'suggested': '#### ' + heading_text,
                    'reason': f'H{deep_h["level"]}→H4로 깊이 축소',
                    'auto_fixable': True,
                })
