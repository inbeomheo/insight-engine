...
## 책임
사용자가 생성한 콘텐츠(분석 히스토리)의 영속화/조회/관리. `ie_histories` 테이블이 단일 권위 저장소.
...
## 외부 ACL
- `IHistoryRepository` — HistoryEntry 영속화 + 페이지네이션 조회 + 삭제/수정

## 유스케이스
- `SaveHistoryEntryUseCase` — 생성 직후 호출
- `ListHistoryEntriesUseCase` — 사용자 히스토리 페이지네이션 조회 (관리자는 전체)
- `UpdateHistoryEntryUseCase` — 마인드맵 캐싱 등 부분 갱신
- `ToggleFavoriteUseCase` — 즐겨찾기 토글
- `DeleteHistoryEntryUseCase` — 삭제

## 의존 방향
- 다른 BC → Content/Library: `IHistoryRepository`, UseCase들
