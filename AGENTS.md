...
**파일 업로드 학습**: `POST /api/extract-document`(PDF/DOCX/PPTX) / `POST /api/extract-audio`(Whisper 전사, `WHISPER_ENABLED` 필요) — 추출·전사 텍스트를 텍스트 탭에 채워 직접 텍스트 경로로 학습. 업로드 상한은 `DOCUMENT_UPLOAD_MAX_BYTES`/`AUDIO_UPLOAD_MAX_BYTES` (경로별 before_request 훅). `/generate`의 multipart 파일 모드는 제거됨 — URL/직접 텍스트 전용
