...
- [ ] [제품] 학습 고도화 — 입력 자료를 요약보다 "학습 가능한 노트"로 구조화.
  ...
  15차: 복습 완료 후 내일·3일·7일 간격으로 다음 복습을 예약하고, `/notes` 학습 큐에 오늘·예정 복습을 기한순으로 표시.
...
- [x] 2026-07-12 feat(notes): 검색→개념→검색 복귀 경로 추가 (PR #125).
  ...
  검증: backend 3,875 passed / 1 skipped / 11 subtests, 대상 7 passed, frontend 161 passed, 전체 ESLint·tsc·build 통과, 독립 리뷰 BLOCKER/IMPORTANT 0·NIT 1(통합 상태 전환 테스트 후속).
...
- [x] 2026-07-11 feat(learning): 주간 복습 기록 Markdown 복사 추가.
  ...
  노트 제목 공백 정리·Markdown 링크 이스케이프·내부 링크 인코딩과 빈 기록을 단위 테스트로 검증.
  검증: frontend 90 passed, 전체 ESLint·tsc·build 통과.

- [x] 2026-07-11 feat(learning): 최근 7일 복습 활동 차트 추가.
  날짜별 완료 횟수를 오래된 날부터 오늘까지 계산하고, 최대 활동일 대비 막대 높이와 오늘 요일 강조를 표시.
  DST 환경에서도 날짜가 어긋나지 않도록 로컬 날짜 증감 방식으로 계산.
  검증: frontend 89 passed, 전체 ESLint·tsc·build 통과.

- [x] 2026-07-11 feat(learning): 복습 기록·연속 학습 통계 추가.
  일정 예약 시 노트별 하루 1회 완료 기록을 로컬 저장하고 같은 날 간격 변경은 최신 기록으로 갱신.
  LLMWiki 학습 큐에 최근 7일 완료 수·활동일·오늘/어제까지 이어지는 연속 학습과 최근 노트 3개를 표시.
  검증: backend 3,873 passed / 1 skipped / 11 subtests, frontend 88 passed, 전체 ESLint·tsc·build 통과.
...
- [x] 2026-07-11 feat(learning): 복습 예정일·오늘 복습 큐 추가.
  ...
  `/notes` 학습 큐에 기한이 지난 일정과 오늘·예정 일정을 우선순위순으로 표시하고 기존 완료·최근 카드와 중복 제거.
  검증: backend 3,873 passed / 1 skipped / 11 subtests, frontend 85 passed, 변경 파일 ESLint·tsc·build 통과.
...
- [x] 2026-07-10 chore(dead-code): 메모리·자동태깅·단계요약 엔드포인트 제거.
  ...
  생성 컨텍스트에서 쓰이는 `memory_service`는 보존하고, 라우트 전용 고아 서비스 `auto_tag_service`, `progressive_summary_service`와 전용 테스트를 삭제.
  검증: `.venv\Scripts\python.exe -m pytest tests/test_advanced_routes_cov.py tests/test_memory_service.py -q` 41 passed +
...
- [x] 2026-07-10 feat(notes): Q&A 복습 카드함 전체 복사 추가.
  ...
  검증: `cd frontend && npm.cmd test -- ResultChatPanel.test.tsx result-chat-study-card.test.ts note-wiki-brief.test.ts note-outline.test.ts note-study-progress.test.ts note-review-session.test.ts note-list.test.ts` 53 passed +
  `cd frontend && npx.cmd tsc --noEmit` 통과 +
  `git diff --check` 통과.
...
- [x] 2026-07-10 feat(notes): Q&A 복습 카드함 추가.
  ...
  검증: `cd frontend && npm.cmd test -- ResultChatPanel.test.tsx result-chat-study-card.test.ts note-wiki-brief.test.ts note-outline.test.ts note-study-progress.test.ts note-review-session.test.ts note-list.test.ts` 50 passed +
  `cd frontend && npx.cmd tsc --noEmit` 통과 +
  `git diff --check` 통과.
- [x] 2026-07-10 feat(notes): 근거 Q&A 복습 카드 복사 추가.
  `/notes/[id]` 근거 기반 채팅 답변에 `복습 카드 복사` 버튼을 추가해 질문·답변·근거 출처를 Markdown 복습 카드로 저장할 수 있게 개선.
  공용 `result-chat-study-card` 유틸을 추가해 Q&A와 근거 출처를 안전한 Markdown으로 정리하고, `ResultChatPanel`은 직전 사용자 질문과 답변을 묶어 클립보드로 복사하도록 확장.
  복습 카드 Markdown 생성과 채팅 답변 복사 동작을 단위 테스트로 검증.
  검증: `cd frontend && npm.cmd test -- ResultChatPanel.test.tsx result-chat-study-card.test.ts note-wiki-brief.test.ts note-outline.test.ts note-study-progress.test.ts note-review-session.test.ts note-list.test.ts` 47 passed +
  `cd frontend && npx.cmd tsc --noEmit` 통과 +
  `git diff --check` 통과.
