...
def test_post_notes_generates_saves_and_returns_note(tmp_path, monkeypatch):
    ...
    with (
        ...
        resp = client.post(
            "/api/notes",
            json={"content": "원문 콘텐츠", "source": _source(), "language": "ko", "model": "cliproxyapi/gpt-5.6-luna"},
            headers=_H,
        )
...
def test_post_notes_generates_from_text_source_without_url(tmp_path, monkeypatch):
    ...
    with (
        ...
        resp = client.post(
            "/api/notes",
            json={"content": "붙여넣은 원문 콘텐츠", "source": source, "language": "ko", "model": "cliproxyapi/gpt-5.6-luna"},
            headers=_H,
        )
