...
class TestDownloadAudio(unittest.TestCase):
    ...
    @patch('services.whisper_service.os.path.getsize', return_value=0)
    @patch('services.whisper_service.os.path.exists', return_value=True)
    def test_empty_audio_file(self, mock_exists, mock_size):
        ...
        with patch.dict('sys.modules', {'yt_dlp': mock_yt_dlp}):
            with patch('services.whisper_service.tempfile.mkstemp', return_value=(5, '/tmp/test.wav')):
                with patch('services.whisper_service.os.close'):
                    with patch('services.whisper_service.os.remove'):
                        result = download_audio('https://youtube.com/watch?v=test')
        self.assertIsNone(result)
...
class TestExtractTranscriptWhisper(unittest.TestCase):
    ...
    @patch('services.whisper_service.transcribe_audio', return_value='변환된 텍스트')
    ...
    @patch('services.whisper_service._cleanup_file')
    @patch('services.whisper_service.download_audio', return_value=None)
    ...
    @patch('services.whisper_service._cleanup_file')
    @patch('services.whisper_service.transcribe_audio', return_value=None)
    @patch('services.whisper_service.download_audio', return_value='/tmp/audio.wav')
    ...
    @patch('services.whisper_service._cleanup_file')
    @patch('services.whisper_service.download_audio', side_effect=Exception('unexpected'))
