...
class TestTrash(_Base):
    """F8-24 휴지통."""

    @patch(_SB_PATCH, return_value=False)
    @patch('services.data.trash_service.list_trash', return_value={'items': [], 'total': 0})
    def test_list_trash(self, _m1, _):
        resp = self.client.get('/api/content/trash', headers=_H)
        self.assertEqual(resp.status_code, 200)

    @patch(_SB_PATCH, return_value=False)
    @patch('services.data.trash_service.restore_from_trash', return_value={'id': 'a'})
    def test_restore_from_trash(self, _m1, _):
        resp = self.client.post('/api/content/trash/a/restore', json={}, headers=_H)
        self.assertEqual(resp.status_code, 200)

    @patch(_SB_PATCH, return_value=False)
    @patch('services.data.trash_service.restore_from_trash', return_value=None)
    def test_restore_from_trash_not_found(self, _m1, _):
        resp = self.client.post('/api/content/trash/bad/restore', json={}, headers=_H)
        self.assertEqual(resp.status_code, 404)

    @patch(_SB_PATCH, return_value=False)
    @patch('services.data.trash_service.permanently_delete', return_value=True)
    def test_permanent_delete(self, _m1, _):
        resp = self.client.delete('/api/content/trash/a', json={}, headers=_H)
        self.assertEqual(resp.status_code, 200)

    @patch(_SB_PATCH, return_value=False)
    @patch('services.data.trash_service.empty_trash', return_value=5)
    def test_empty_trash(self, _m1, _):
        resp = self.client.post('/api/content/trash/empty', json={}, headers=_H)
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.get_json()['permanently_deleted'], 5)
...
class TestPin(_Base):
    ...
    @patch(_SB_PATCH, return_value=False)
    @patch('services.data.content_library_service.update_item', return_value={'id': 'a', 'is_pinned': True})
    def test_pin_content(self, _m1, _):
        resp = self.client.post('/api/content/a/pin', json={}, headers=_H)
        self.assertEqual(resp.status_code, 200)
        self.assertTrue(resp.get_json()['pinned'])

    @patch(_SB_PATCH, return_value=False)
    @patch('services.data.content_library_service.update_item', return_value=None)
    def test_pin_not_found(self, _m1, _):
        resp = self.client.post('/api/content/missing/pin', json={}, headers=_H)
        self.assertEqual(resp.status_code, 404)

    @patch(_SB_PATCH, return_value=False)
    @patch('services.data.content_library_service.update_item', return_value={'id': 'a', 'is_pinned': False})
    def test_unpin_content(self, _m1, _):
        resp = self.client.delete('/api/content/a/pin', headers=_H)
        self.assertEqual(resp.status_code, 200)

    @patch(_SB_PATCH, return_value=False)
    @patch('services.data.content_library_service.update_item', return_value=None)
    def test_unpin_not_found(self, _m1, _):
        resp = self.client.delete('/api/content/missing/pin', headers=_H)
        self.assertEqual(resp.status_code, 404)

    @patch(_SB_PATCH, return_value=False)
    @patch('services.data.content_library_service.search_items', return_value={'items': [], 'total': 0})
    def test_list_pinned(self, _m1, _):
        resp = self.client.get('/api/content/pinned', headers=_H)
        self.assertEqual(resp.status_code, 200)
