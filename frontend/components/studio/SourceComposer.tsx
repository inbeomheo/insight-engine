    ...
    <section className="rounded-[24px] border border-slate-200/80 bg-white p-4 shadow-sm shadow-slate-200/60 sm:p-5">
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <div className="flex rounded-full bg-slate-100 p-1 text-xs font-medium">
          <button type="button" data-testid="source-tab-url" onClick={() => setTab('url')} className={cn('flex items-center gap-1.5 rounded-full px-3 py-1.5', tab === 'url' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500')}><Link2 className="h-3.5 w-3.5" /> URL</button>
          <button type="button" data-testid="source-tab-text" onClick={() => setTab('text')} className={cn('flex items-center gap-1.5 rounded-full px-3 py-1.5', tab === 'text' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500')}><FileText className="h-3.5 w-3.5" /> 텍스트</button>
        </div>
      </div>
      {tab === 'url' ? (
