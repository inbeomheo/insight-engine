    ...
    >
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="max-w-xl">
          ...
          <h3 className="mt-2 text-xl font-semibold text-slate-950">아직 결과가 없습니다</h3>
          <p className="mt-2 text-sm leading-6 text-slate-500">
            왼쪽부터 순서대로 진행하면 입력부터 후처리까지 한 번에 이어집니다.
          </p>
          <Button type="button" className="mt-4 rounded-2xl" onClick={onFocusSource}>
            소스 입력으로 이동
          </Button>
        </div>
        ...
        <div className="grid flex-1 gap-2 sm:grid-cols-2">
          {GUIDE_STEPS.map((step) => {
            ...
            return (
              <div key={step.label} className="rounded-2xl border border-slate-200 bg-slate-50/80 p-3">
