...
def create_app(test_config=None):
    ...
    def _set_extract_upload_limit():
        ...
        try:
            ...
            app.logger.warning(
                'Flask <3.1: per-request upload limit unavailable — extract uploads stay capped at the global limit'
            )
        return None
    ...
    def _log_response_time(response):
        ...
        if start is not None:
            ...
            reported_ms = max(elapsed_ms, 0.001)
            response.headers['X-Response-Time'] = f'{reported_ms:.3f}ms'
