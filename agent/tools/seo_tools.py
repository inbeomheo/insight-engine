...
_SERVICE_DIR = os.path.join(os.path.dirname(__file__), "..", "..", "services", "seo")
...
def _register_tools():
    ...
    for _, module_name, _ in pkgutil.iter_modules([_SERVICE_DIR]):
        ...
        if module_name in _SKIP_MODULES:
            continue
        try:
            mod = importlib.import_module(f"services.seo.{module_name}")
            for name, fn in inspect.getmembers(mod, inspect.isfunction):
                ...
                registry.register(
                    name=name, toolset="seo", description=doc,
                    parameters=params, handler=make_handler(fn, param_names),
                )
                count += 1
                break
        except Exception as e:
            logger.debug("seo 모듈 스킵: %s — %s", module_name, e)
    logger.info("SEO 도구 자동 등록: %d개", count)
