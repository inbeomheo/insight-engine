"""Integrations 도구 자동 래핑 — services/integrations/ 서비스를 Tool로 등록"""
...
_SERVICE_DIR = os.path.join(os.path.dirname(__file__), "..", "..", "services", "integrations")
_SKIP_MODULES = set()
...
def _register_tools():
    ...
    for _, module_name, _ in pkgutil.iter_modules([_SERVICE_DIR]):
        if module_name.startswith("_") or module_name in _SKIP_MODULES:
            continue
        try:
            mod = importlib.import_module(f"services.integrations.{module_name}")
            ...
            logger.debug("[integrations] skip: %s — %s", module_name, e)
    logger.info("[Integrations] tools registered: %d", count)
