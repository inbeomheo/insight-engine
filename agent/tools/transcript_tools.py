"""Transcript 도구 자동 래핑 — services/transcript/ 서비스를 Tool로 등록"""
...
_SERVICE_DIR = os.path.join(os.path.dirname(__file__), "..", "..", "services", "transcript")
_SKIP_MODULES = {"whisper_service"}  # core에서 이미 래핑됨
...
def _register_tools():
    ...
    for _, module_name, _ in pkgutil.iter_modules([_SERVICE_DIR]):
        if module_name.startswith("_") or module_name in _SKIP_MODULES:
            continue
        try:
            mod = importlib.import_module(f"services.transcript.{module_name}")
            ...
            logger.debug("[transcript] skip: %s — %s", module_name, e)
    logger.info("[Transcript] tools registered: %d", count)
