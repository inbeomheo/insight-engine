...
class ToolRegistry:
    ...
    def get_schemas(
        ...
        return [e.schema for e in self.list_tools(toolsets=toolsets, available_only=available_only)]
    ...
    def dispatch(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            elapsed = time.time() - start
            logger.error("도구 실행 실패: %s (%.2f초) — %s", name, elapsed, e)
            return json.dumps({"error": f"도구 실행 실패: {e}"}, ensure_ascii=False)
